	...
	<?php if ( $title && $yith_wapo_frontend->_option_show_label_type == 'yes' ) : ?>
		...
		<<?php echo $addon_title_tag; ?>>
			...
			<?php echo ( $sold_individually ? '<abbr class="sold_individually"> (' . __( 'Sold individually', 'yith-woocommerce-product-add-ons' ) . ')</abbr>' : '' ); ?>
                <?php
                if($type_id==36){//BRA SIZE option
                    ?>
                    <span class="cta about_size bra_size_info_modal"><?php echo __('About Size', 'zoa')?></span>
                <?php
                }
                ?>
                <?php
                if($type_id==37){//PANTY SIZE option
                    ?>
                    <span class="cta about_size panty_size_info_modal"><?php echo __('About Size', 'zoa')?></span>
                <?php
                }
                ?>
                <?php
                if($type_id==39){//CAMI SIZE option
                    ?>
                    <span class="cta about_size cami_size_info_modal"><?php echo __('About Size', 'zoa')?></span>
                <?php
                }
                ?>
                <?php
                if($type_id==40){//SHORTS SIZE option
                    ?>
                    <span class="cta about_size shorts_size_info_modal"><?php echo __('About Size', 'zoa')?></span>
                <?php
                }
                ?>
		</<?php echo $addon_title_tag; ?>>
