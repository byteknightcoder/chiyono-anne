...
?><h4 class="bundled_product_title product_title">
	 ...
	 */
    $link = apply_filters('woocommerce_bundled_item_link_html', ' <span class="bundled_product_title_link"><a class="bundled_product_permalink" href="' . get_permalink($bundled_item->get_product_id()) . '" target="_blank" aria-label="' . esc_attr__('View product', 'woocommerce-product-bundles') . '"></a></span>', $bundled_item, $bundle); // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
	echo wp_kses_post( $title . $link );
    ?>
</h4>
<?php
if ( function_exists('the_subtitle') ) {
    if ( '' != get_the_subtitle( $bundled_item->get_product_id() ) ) {
        echo '<span class="subtitle">' . get_the_subtitle($bundled_item->get_product_id()) . '</span>';
    }
}
