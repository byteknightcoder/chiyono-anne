...
<div class="<?php echo esc_attr( implode( ' ', $gallery_classes ) ); ?>">
	...
	if ( has_post_thumbnail( $product_id ) ) {
		...
		$image_title   = esc_attr( get_the_title_product_chiyono( $image_post_id ) );
		$image_data    = wp_get_attachment_image_src( $image_post_id, 'full' );
		$image_link    = $image_data[0];
        $p = wc_get_product($product_id);
        $product_visibility = $p->get_catalog_visibility();
        $p_link = '';
        ...
        // $html .= sprintf('<a href="%1$s" class="image ch ' . $p_link . '" title="%2$s" data-rel12="%3$s">%4$s</a>', $p_link, $image_title, $image_rel, $image);
        if ( 'hidden' == $product_visibility ) {
            $html .= '<aa href="javascript:;" class="image" title="' . $image_title . '" data-rel="' . $image_rel . '">' . $image . '</a>';
        } else {
            $p_link = $p->get_permalink();
            $html .= '<a href="' . $p_link . '" ch_bdl_link="' . $p_link . '" class="image ch_product_bdl_link" title="' . $image_title . '" data-rel="' . $image_rel . '">' . $image . '</a>';
        }
        $html .= '</figure>';
    } else {
