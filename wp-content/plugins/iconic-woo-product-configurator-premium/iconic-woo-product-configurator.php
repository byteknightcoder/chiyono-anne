...
class jckpc {
	...
	public function display_product_image( $show_thumbnails = true, $product_id = null ) {
		...
		$querySelectors = $_REQUEST;

		if ( ! empty( $querySelectors ) && is_product()) {
			$sanitisedQuerySelectors = array();

			foreach ( $querySelectors as $attSlug => $attVal ) {
                            if(!empty($attSlug)&&!empty($attVal))
	...
	public function get_image_layer() {
		...
		if ( ! empty( $selected_val ) ) {
			...
			if ( ! empty( $imgID ) ) {
				$response['image'] = wp_get_attachment_image(
					$imgID,
					$image_size,
					false,
					array(
	...
	public function get_atts_from_querystring( $prodid = false, $qarr = false ) {
		...
		if ( isset( $qarr['jckpc-img-size'] ) ) {
			$atts['jckpc-img-size'] = $qarr['jckpc-img-size'];
		}
	...
	public function get_images_from_chosen_atts( $setImages, $chosenAtts ) {
		$images = array();
		$size   = isset( $_REQUEST['jckpc-img-size'] ) ? $_REQUEST['jckpc-img-size'] : 'full';
		...
		if ( is_array( $chosenAtts ) ) {
			foreach ( $chosenAtts as $attSlug => $attVal ) {
				...
				$images[ $attSlug ] = $this->get_attachment_image_path( $setImages[ $attSlug ][ $attVal ], $size );
			}
		}

		if ( isset( $setImages['background'] ) && $setImages['background'] != "" ) {
			$images['background'] = $this->get_attachment_image_path( $setImages['background'], $size );
		}
	...
	public function get_product_image_url( $imgData ) {
		foreach ( $imgData as $key => $value ) {
			if ( $key === 'jckpc-img-size' || strpos( $key, 'jckpc-' ) === false ) {
				continue;
			}
		...
		if ( false === get_transient( $imgPaths['imgName'] ) ) {
			$imgData['action'] = 'jckpc_generate_image';
			return add_query_arg( array_filter( $imgData ), admin_url( 'admin-ajax.php' ) );
		} else {
	...
	public function cart_thumbnail( $thumb, $cart_item, $cart_item_key = false ) {
		...
		if ( $configurator_enabled ) {
			...
			$imgData['jckpc-img-size'] = 'woocommerce_thumbnail';
