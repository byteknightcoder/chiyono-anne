...
class Iconic_PC_Core_Settings {
	...
	public static function hide_notices() {
		$hide_notice = filter_input( INPUT_GET, 'jckpc-hide-notice' );
	...
	public static function account_getting_started() {
		...
		if ( ! defined( 'ICONIC_PC_DISABLE_DASH' ) || ( class_exists( 'Iconic_PC_Core_Licence' ) && ! Iconic_PC_Core_Licence::has_valid_licence() ) ) {
			return;
		}
		...
		<div class="iconic-notice iconic-notice--getting-started">
			<a class="woocommerce-message-close notice-dismiss" href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'jckpc-hide-notice', $option_name ), self::$args['option_group_underscore'] . '_hide_notices_nonce', '_' . self::$args['option_group_underscore'] . '_notice_nonce' ) ); ?>">
				<?php esc_html_e( 'Dismiss', 'woocommerce' ); ?>
			</a>
			<h2>Welcome to <?php echo esc_html( self::$args['title'] ); ?>!</a></h2>
			<p><?php esc_html_e( "Thank you for choosing Iconic. We've put together some useful links to help you get started:", 'jckpc' ); ?></p>

			<?php self::output_getting_started_links(); ?>

			<p>
				<strong><?php esc_html_e( 'Need some help?', 'jckpc' ); ?></strong>
				<?php
				printf(
				/* Translators: Documentation URL. */
					wp_kses_post( __( 'Take a look at our <a href="%s?utm_source=Iconic&utm_medium=Plugin&utm_campaign=jckpc&utm_content=need-some-help" target="_blank">troubleshooting documentation</a>.', 'jckpc' ) ),
					esc_url( self::get_docs_url( 'troubleshooting' ) )
				);
				?>
				<?php esc_html_e( 'There is a permanent link to the plugin documentation in the "Support" section below.', 'jckpc' ); ?>
			</p>
		</div>
		<?php
	}
	...
	public static function output_getting_started_links() {
		...
		?>
		<h3><?php esc_html_e( 'Getting Started', 'jckpc' ); ?></h3>

		<ol>
			<?php foreach ( $links as $link ) { ?>
				<li>
					<a href="<?php echo esc_url( self::get_docs_url() . $link['href'] ); ?>?utm_source=Iconic&utm_medium=Plugin&utm_campaign=jckpc&utm_content=getting-started-links" target="_blank"><?php echo esc_html( $link['title'] ); ?></a>
				</li>
			<?php } ?>
		</ol>
		<?php
	...
	public static function setup_dashboard( $settings ) {
		...
		if ( current_user_can( 'manage_options' ) && ! defined( 'ICONIC_DISABLE_DASH' ) && ! defined( 'ICONIC_PC_DISABLE_DASH' ) ) {
	...
	public static function add_sidebar() {
		...
		$cross_sells  = Iconic_PC_Core_Cross_Sells::get_selected_plugins( self::$args['cross_sells'] );
		$show_bundle  = ! Iconic_PC_Core_Licence::is_bundle() && Iconic_PC_Core_Licence::get_license_quota() <= 3;
		$is_trial     = Iconic_PC_Core_Licence::is_trial();
		...
		<div class="iconic-settings-sidebar">
			<?php if ( $is_trial ) { ?>
				<div class="iconic-settings-sidebar__widget iconic-settings-sidebar__widget--note">
					...
					<a href="https://iconicwp.com/support/?utm_source=Iconic&utm_medium=Plugin&utm_campaign=jckpc&utm_content=trial-cta" class="button button-primary" target="_blank">Get in touch</a>
					<a href="<?php echo esc_attr( self::get_docs_url( 'collection' ) ); ?>?utm_source=Iconic&utm_medium=Plugin&utm_campaign=jckpc&utm_content=trial-cta" class="button button-secondary" target="_blank">View docs</a>
			...
			<?php if ( $show_bundle ) { ?>
				<div class="iconic-settings-bundle iconic-settings-sidebar__widget iconic-settings-sidebar__widget--bundle">
					...
					<p class="iconic-settings-bundle__secondary-link">or <a href="https://iconicwp.com/bundles/?utm_source=Iconic&utm_medium=Plugin&utm_campaign=jckpc&utm_content=bundles-cta" target="_blank">view bundles</a></p>
					...
					<?php foreach ( $cross_sells as $cross_sell ) { ?>
						<div class="iconic-product">
							...
							<div class="iconic-product__content">
								<h4 class="iconic-product__title"><a target="_blank" href="<?php echo esc_url( $cross_sell['url'] ); ?>?utm_source=Iconic&utm_medium=Plugin&utm_campaign=jckpc&utm_content=cross-sell" target="_blank"><?php echo esc_html( $cross_sell['title'] ); ?></a></h4>
	...
	public static function support_link() {
		return sprintf( '<a href="%s" class="button button-secondary" target="_blank">%s</a>', 'https://iconicwp.com/support/?utm_source=Iconic&utm_medium=Plugin&utm_campaign=jckpc&utm_content=support-link', __( 'Submit Ticket', 'jckpc' ) );
	}
	...
	public static function documentation_link() {
		return sprintf( '<a href="%s?utm_source=Iconic&utm_medium=Plugin&utm_campaign=jckpc&utm_content=documentation-link" class="button button-secondary" target="_blank">%s</a>', self::get_docs_url( 'collection' ), __( 'Read Documentation', 'jckpc' ) );
	}
	...
	public static function account_link() {
		return sprintf( '<a href="%s" class="button button-secondary" target="_blank">%s</a>', 'https://iconicwp.com/account/?utm_source=Iconic&utm_medium=Plugin&utm_campaign=jckpc&utm_content=account-link', __( 'Manage Your Account', 'jckpc' ) );
	}
