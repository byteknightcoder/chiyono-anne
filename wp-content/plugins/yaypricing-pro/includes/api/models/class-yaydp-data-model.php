			...
			$customers = \array_map(
				function( $item ) {
					return array(
						...
						'first_name' => get_user_meta( $item->id, 'first_name' ),
						'last_name'  => get_user_meta( $item->id, 'last_name' ),
						'label' => get_user_meta( $item->id, 'first_name' ) . get_user_meta( $item->id, 'last_name' ) . "(#" . $item->user_email . ")"
					);
				},
				$query_results
			);
