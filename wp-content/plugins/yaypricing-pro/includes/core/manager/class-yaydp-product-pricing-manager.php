...
class YAYDP_Product_Pricing_Manager {
	...
	private function __construct() {
		...
		add_action( 'woocommerce_before_add_to_cart_form', array( $this, 'add_offer_description' ), 10 );
		/*
		add_action( 'woocommerce_cart_totals_after_order_total', function() {
			...
			if ( ! is_null( $yaydp_cart ) ) { 
				...
				?>
			<tr>
				<th><?php _e( 'Product discounts', 'yaydp' ); ?></th>
		...
		*/
		...
		// Handle encouraged notice.
		$this->handle_encouraged_notice();

		// Handle countdown.
		$this->handle_countdown();
	 ...
	 * Handle encouraged notice
	 */
	private function handle_encouraged_notice() {
		\YAYDP\Core\Encouraged_Notice\YAYDP_Product_Pricing_Encouraged_Notice_Handler::get_instance();
	}

	/**
	 * Handle countdown
	 */
	private function handle_countdown() {
		\YAYDP\Core\Countdown\YAYDP_Product_Pricing_Countdown_Handler::get_instance();
	}
