<?php
/**
 * Managing handler for cart discount encouraged notice
 *
 ...
 * Declare class
 */
class YAYDP_Cart_Discount_Encouraged_Notice_Handler {
	...
	protected function __construct() {
		add_action( 'yaydp_bottom_cart_discount_encouraged_section', array( $this, 'add_bottom_encouraged_notice' ), 100 );
		add_shortcode( 'yaypricing-cart-discount-encouraged-notice', [ $this, 'encouraged_notice_shortcode' ] );
	}
	...
	public function can_show_at_bottom() {
		$is_enabled = \YAYDP\Settings\YAYDP_Cart_Discount_Settings::get_instance()->is_enabled_encouraged_notice();
		if ( ! $is_enabled ) {
			return false;
		}
	...
	public function add_bottom_encouraged_notice() {
		...
		echo wp_kses_post( do_shortcode('[yaypricing-cart-discount-encouraged-notice]') );
	}
	...
	public function encouraged_notice_shortcode() {
		...
		if ( empty( $bottom_encouragement ) ) {
			return '<div class="cart-discount-encouraged-notice"></div>';
		}

		ob_start();
		?>
		<div class="cart-discount-encouraged-notice">
			<?php
			\wc_get_template(
