...
class YAYDP_Cart_Item {
	...
	public function __construct( $cart_item_data ) {
		...
		if ( ! empty( $cart_item_data['custom_price'] ) ) {
			$this->price = $cart_item_data['custom_price'];
		} else {
			$this->price = \YAYDP\Helper\YAYDP_Pricing_Helper::get_product_price( $this->product );
		}
	}
	...
	private function get_modified_price_html_for_normal_item() {
		...
		$origin_price            = \YAYDP\Helper\YAYDP_Pricing_Helper::get_product_price( $this->get_product() );
