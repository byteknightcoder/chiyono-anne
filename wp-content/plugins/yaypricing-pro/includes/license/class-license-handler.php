...
class License_Handler {
	...
	public function not_activate_license_notice() {
		...
		$license = new License( 'yaypricing' );
	...
	<div class="error">		
		...
		printf( esc_html__( '%1$s license key is required. %2$sPlease enter your license key to start using the plugin%3$s.', 'yaypricing' ), 'YayPricing', '<a href="' . esc_url( admin_url( 'admin.php?page=yaycommerce-licenses' ) ) . '">', '</a>' );
		?>
		</p>
	</div>
		<?php
	...
	public function get_version_cron_run() {
		$licensing_plugins = $this->get_licensing_plugins();
		foreach ( $licensing_plugins as $plugin ) {
			$licensing_plugin = new Licensing_Plugin( $plugin['slug'] );
	...
	public function enqueue_license_scripts() {
		...
		wp_enqueue_script( Core_Plugin::get( 'slug' ) . '-license-script', plugin_dir_url( __FILE__ ) . 'assets/js/license.js', array(), Core_Plugin::get( 'version' ), true );
		wp_localize_script(
			Core_Plugin::get( 'slug' ) . '-license-script',
			Core_Plugin::get( 'slug' ) . 'LicenseData',
			array(
				'apiSettings' => array(
					'restNonce' => wp_create_nonce( 'wp_rest' ),
					'restUrl'   => esc_url_raw( rest_url( Core_Plugin::get( 'slug' ) . '-license/v1' ) ),
					'adminUrl'  => admin_url(),
				),
			)
		);
	...
	public function yaycommerce_licenses_settings() {
		...
		foreach ( $licensing_plugins as $_plugin ) {
			$licensing_plugin = new Licensing_Plugin( $_plugin['slug'] );
	...
	public static function get_licensing_plugins() {
		$plugin_data = self::get_plugin_data(
			array(
				'dir_path' => Core_Plugin::get( 'path' ),
				'basename' => Core_Plugin::get( 'basename' ),
			)
		);
		$plugins     = array(
			array(
				'name'     => $plugin_data['Name'],
				'slug'     => Core_Plugin::get( 'slug' ),
				'dir_path' => Core_Plugin::get( 'path' ),
				'basename' => Core_Plugin::get( 'basename' ),
				'file'     => Core_Plugin::get( 'path' ) . basename( Core_Plugin::get( 'basename' ) ),
				'url'      => Core_Plugin::get( 'link' ),
				'item_id'  => Core_Plugin::get( 'download_id' ),
			),
		);
		return apply_filters( Core_Plugin::get( 'slug' ) . '_available_licensing_plugins', $plugins );
	}
	...
	public function do_remove_license_request() {
		...
		$licensing_plugin_slug = isset( $_POST[ Core_Plugin::get( 'slug' ) . '_licensing_plugin' ] ) ? sanitize_text_field( $_POST[ Core_Plugin::get( 'slug' ) . '_licensing_plugin' ] ) : '';
	...
	public function do_post_requests() {
		if ( isset( $_SERVER['REQUEST_METHOD'] ) && 'POST' === $_SERVER['REQUEST_METHOD'] ) {
			if ( isset( $_POST['_wpnonce'] ) && wp_verify_nonce( sanitize_text_field( $_POST['_wpnonce'] ), 'woocommerce-settings' ) ) {
				if ( isset( $_POST[ Core_Plugin::get( 'slug' ) . '_remove_license' ] ) ) {
			...
			function() use ( $plugin ) {
				...
				<script>
					jQuery(document).ready(function(){
						...
						update_a_tag.id = '<?php echo esc_attr( Core_Plugin::get( 'slug' ) ); ?>-activate-license';
