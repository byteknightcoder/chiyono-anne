<?php
/**
 * Plugin Name: YayPricing Pro - WooCommerce Dynamic Pricing And Discounts
 * Plugin URI: https://yaycommerce.com/yaypricing-woocommerce-dynamic-pricing-and-discounts/
...
if ( ! function_exists( 'YAYDP\\load_plugin' ) ) {
	...
	function load_plugin() { //phpcs:ignore
		...
		if ( function_exists( 'WC' ) && \version_compare( phpversion(), YAYDP_MINIMUM_PHP_VERSION, '>=' ) ) {
			\YAYDP\License\License_Handler::get_instance();
			$license = new \YAYDP\License\License( 'yaypricing' );
...
if ( ! function_exists( 'YAYDP\\before_load_plugin' ) ) {
	...
	function before_load_plugin() {
		...
		\YAYDP\License\License_Handler::get_instance(); // Initialize License.
