...
class WCCM_CustomerTable extends WP_List_Table {
    ...
    function column_login($item) {
        ...
        $rank_and_amount=mr_get_member_rank($item['ID'],true);
        return @sprintf('<a href="?page=' . $_REQUEST['page'] . '&customer=' . $item['ID'] . '&action=customer_details">' .
                        get_avatar($item['ID'], 64, "", false, array('class' => 'wccm_avatar_img')) . '
								<div class="wccm_customer_data_container">
                                                                <span class="wccm_login_container user_rank">' . $rank_and_amount['rank'] . '(&yen;'. number_format($rank_and_amount['spent_total_in_current']).')</span>
    ...
    function months_and_roles_dropdown($post_type = 'shop_order') {
        ...
        <div id="wccm_date_filter_box">
            ...
            <?php
            if(isset($_SESSION['user_imported'])&&!isset($_REQUEST['user_imported'])){
                $reg_type = $_SESSION['user_imported'];
            }else{
                $reg_type = $_REQUEST['user_imported'];
                $_SESSION['user_imported']=$reg_type;
            }
            if(isset($_SESSION['member_rank'])&&!isset($_REQUEST['member_rank'])){
                $rank = $_SESSION['member_rank'];
            }else{
                $rank = $_REQUEST['member_rank'];
                $_SESSION['member_rank']=$rank;
            }
            ?>
            <select name="user_imported">
                <option <?php if ($reg_type=='') echo 'selected="true"'; ?> value="">All type</option>
                <option <?php if ($reg_type=='yes') echo 'selected="true"'; ?>  value="yes">User Imported</option>
                <option <?php if ($reg_type=='no') echo 'selected="true"'; ?>  value="no">User Registered</option>
            </select>
            <input class="button-primary" type="submit" name="reg_type_btn" value="<?php _e('Filter By Register Type', 'woocommerce-customers-manager'); ?>" />
            <select name="member_rank">
                <option <?php echo $rank == '' ? 'selected' : ''; ?> value="">All</option>
                <option <?php echo $rank == 'royal' ? 'selected' : ''; ?> value="royal"><?php _e('royal', 'zoa'); ?></option>
                <option <?php echo $rank == 'gold' ? 'selected' : ''; ?> value="gold"><?php _e('gold', 'zoa'); ?></option>
                <option <?php echo $rank == 'silver' ? 'selected' : ''; ?> value="silver"><?php _e('silver', 'zoa'); ?></option>
                <option <?php echo $rank == 'bronze' ? 'selected' : ''; ?> value="bronze"><?php _e('bronze', 'zoa'); ?></option>
            </select>
            <input class="button-primary" type="submit" name="member_rank_btn" value="<?php _e('Filter By Member Rank', 'woocommerce-customers-manager'); ?>" />
        </div>
        <?php
        if($rank!=''||isset($_SESSION['member_rank'])){
            ?>
<!--    <style>
    .displaying-num {
...
<?php
    ...
    function render_page() {
        ...
        <form id="export_csv" method="post">
        <?php
                    $o_type=array(
            'オンラインストア',
            'アトリエ',
         ...
         ?>
            <select name="ch_order_type_woo">
                <?php
                foreach ($o_type as $value) {
                    $selected='';
                    if($value==$_REQUEST['ch_order_type_woo']){
            ...
            <input class="button-primary" type="submit" name="btn_export_csv_by_order" value="<?php _e('Export CSV By Order Type', 'woocommerce-customers-manager'); ?>" />
        </form>
...
add_action('admin_init', 'ch_export_customer_csv',99);

function ch_export_customer_csv() {
    if(isset($_REQUEST['btn_export_csv_by_order'])){
        $ch_order_type=$_REQUEST['ch_order_type_woo'];
        $args = array(
           'posts_per_page' => -1,
            'post_status'=>'any',
        ...
        if ($the_query->have_posts()) {
            $file_name = date('YmdHis') . '_customers_info.csv';
            header('Content-Type: application/csv');
            header('Content-Disposition: attachment; filename="' . $file_name . '";');
            $output = fopen('php://output', 'w');
            fputcsv($output, array('billing_email', 'billing_last_name', 'billing_first_name', 'paid_date'));
            while ($the_query->have_posts()) {
                $the_query->the_post();
                $email=get_post_meta(get_the_ID(), '_billing_email',true);
                if(!empty($email)){
                    $_billing_last_name=get_post_meta(get_the_ID(), '_billing_last_name',true);
                    $_billing_first_name=get_post_meta(get_the_ID(), '_billing_first_name',true);
                    $_paid_date=get_the_date('Y-m-d');
                    if(!in_array($email,$data_emails)){
                        $data_emails[]=$email;
                        fputcsv($output, array($email, $_billing_last_name, $_billing_first_name, $_paid_date));
                    }
                }
            }
            wp_reset_query();
