...
class WCCM_Customer {
    ...
    private function add_blocked_role() {
        ...
        $result = !$exist ? add_role(
                        'blocked_customer', $this->get_blocked_role_name()
                ) : true;
    }
...
//wccm_var_dump($order->post_status);
    ...
    public function get_all_users_with_orders_ids($current_page, $per_page, $customers_ids_to_use_as_filter = null, $get_total = false, $ids = null, $starting_date = null, $ending_date = null, $filter_by_product_id = null, $is_for_export = false) {
        ...
        if ($is_for_export) {
            ...
            switch ($orderby) {
                case 'total_sales': $_GET['orderby'] = 'total_spent';
                    break;
                case 'count(DISTINCT posts.ID)': $_GET['orderby'] = 'orders';
                    break;
                case 'users.user_registered': $_GET['orderby'] = 'registered';
                    break;
                case 'users.ID': $_GET['orderby'] = 'ID';
                    break;
                case 'usermeta_name.meta_value': $_GET['orderby'] = 'name';
                    break;
                case 'usermeta_surname.meta_value': $_GET['orderby'] = 'surname';
                    break;
                case 'users.user_email': $_GET['orderby'] = 'email';
                    break;
                case 'posts.post_date': $_GET['orderby'] = 'users.user_registered' /* 'last_order_date' */;
                    break;
                case 'users.user_login': $_GET['orderby'] = 'login';
                    break;
            }
            $_GET['order'] = $order;
        ...
        if (!$get_total) {
            ...
            if(isset($_SESSION['member_rank'])&&!isset($_REQUEST['member_rank'])){
                $member_rank = $_SESSION['member_rank'];
            }else{
                $member_rank = $_REQUEST['member_rank'];
                $_SESSION['member_rank']=$member_rank;
            }
            if($reg_type=='yes'){
            $user_ids_query.=" AND users.ID IN (SELECT user_id FROM {$wpdb->usermeta} AS usermeta2 WHERE usermeta2.meta_key='is_user_imported' AND usermeta2.meta_value='yes') ";
            }elseif($reg_type=='no'){
                $user_ids_query.=" AND users.ID NOT IN (SELECT user_id FROM {$wpdb->usermeta} AS usermeta2 WHERE usermeta2.meta_key='is_user_imported' AND usermeta2.meta_value='yes') ";
            }
            ...
            $id_u=array();
             ...
             if(!empty($id_u)){
                 $user_ids_query.= " AND users.ID IN ('" . implode("','", $id_u) . "') ";
             }
            ...
            $user_ids_query = " SELECT DISTINCT users.id as total ";
...
//            return $result[0];
            ...
            if (isset($user_ids_to_process)) {
                foreach ((array) $user_ids_to_process as $ids_to_process) {
                    $user_member_rank=mr_get_member_rank($ids_to_process->total,true);
                    if(!empty($member_rank)){
                        if($member_rank==$user_member_rank['rank']){
                            $ids[] = $ids_to_process->total;
                        }
                    }else{
                        $ids[] = $ids_to_process->total;
                    }
    ...
    public function get_wpuef_field_names() {
        ...
        $wpuef_column_titles[] = 'billing_last_name_kana';
        $wpuef_column_titles[] = 'billing_first_name_kana';
        $wpuef_column_titles[] = 'shipping_last_name_kana';
        $wpuef_column_titles[] = 'shipping_first_name_kana';
        $wpuef_column_titles[] = 'birthday';
        $wpuef_column_titles[] = 'phone';
