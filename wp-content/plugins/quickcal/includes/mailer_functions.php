...
function quickcal_mailer_tokens()
{
	return apply_filters('quickcal_mailer_tokens', array(
		...
		'reminder_time' => esc_html__("Time until the appointment..", "booked"),
		'template_reminder' => esc_html__("Template for appointment reminder.", "booked"),
		'template_confirmation' => esc_html__("Template for appointment confirmation.", "booked"),
		'template_approval' => esc_html__("Template for appointment approval.", "booked"),
		'template_cancellation' => esc_html__("Template for appointment cancellation.", "booked"),
	));
}
...
function replacePlaceholders($html, $variables)
{
	...
	return str_replace($placeholders, $values, $html);
}


function quickcal_get_appointment_tokens($appt_id, $is_reminder = false)
{
	...
	if ($first_name = get_post_meta($appt_id, '_appointment_guest_name', true)):
		...
		$first_name = get_user_meta($appt_author, 'first_name', true);
		$last_name = get_user_meta($appt_author, 'last_name', true);
		$customer_name = ($last_name ? $last_name . ' ' . $first_name : $first_name);
	...
	$days_until_appointment = floor($time_difference / DAY_IN_SECONDS);

	// Return formatted result - either n days or 1 hour
	if ($days_until_appointment > 0) {
		$reminder_time = $days_until_appointment . ' 日'; // 日 for days
	} else {
		$reminder_time = '1 時間'; // 1 hour if less than 24 hours remain
	}

	// Added by Amin Shoukat
	if ( $is_reminder ) {
		$seconds_diff = $timestamp - $current_timestamp;                            
		$time_diff = ( $seconds_diff / 3600);

		// If 24 hours reminder
		if ( $time_diff <= 24 && $time_diff >= 23 ) {
			$reminder_time = '1 日'; // 日 for days
		}
	...
	if ($calendar_name == "2024年大阪受注会のご予約") {
		$template_reminder = replacePlaceholders( (function_exists('get_field') ? get_field('email_reminder', 2046134) : ''), $variables);
		$template_confirmation = replacePlaceholders( (function_exists('get_field') ? get_field('email_confirm', 2046134) : ''), $variables);
		$template_approval = replacePlaceholders( (function_exists('get_field') ? get_field('email_approval', 2046134) : ''), $variables);
		$template_cancellation = replacePlaceholders( (function_exists('get_field') ? get_field('email_cancel', 2046134) : ''), $variables);
	} else if ($calendar_name == "T.B.Y.B オンラインコンサルテーションのご予約") {
		$template_reminder = replacePlaceholders( (function_exists('get_field') ? get_field('email_reminder', 36861) : ''), $variables);
		$template_confirmation = replacePlaceholders( (function_exists('get_field') ? get_field('email_confirm', 36861) : ''), $variables);
		$template_approval = replacePlaceholders( (function_exists('get_field') ? get_field('email_approval', 36861) : ''), $variables);
		$template_cancellation = replacePlaceholders( (function_exists('get_field') ? get_field('email_cancel', 36861) : ''), $variables);
	} else {
		$template_reminder = replacePlaceholders( (function_exists('get_field') ? get_field('email_reminder', 7995) : ''), $variables);
		$template_confirmation = replacePlaceholders( (function_exists('get_field') ? get_field('email_confirm', 7995) : ''), $variables);
		$template_approval = replacePlaceholders( (function_exists('get_field') ? get_field('email_approval', 7995) : ''), $variables);
		$template_cancellation = replacePlaceholders( (function_exists('get_field') ? get_field('email_cancel', 7995) : ''), $variables);
	}
	...
	return apply_filters('booked_appointment_tokens', array(
		...
		'template_cancellation' => $template_cancellation
	));
...
function get_covid_notify()
{
	ob_start();
	?>
	<table style="padding: 0; margin: 0;" border="0" width="100%" cellspacing="0" cellpadding="0" align="center"
		bgcolor="#ffffff">
		<tbody>
			<tr>
				<td style="height:15px;border-top: 1px solid #eeeeee;">&nbsp;</td>
			</tr>
			<tr>
				<td style="margin: 0; padding-top: 15px; padding-bottom: 0; font-size: 14px; font-weight: normal; color: #000000; font-family: 'Noto Sans JP', sans-serif; , serif; line-height: 24px; mso-line-height-rule: exactly;"
					align="left" valign="top">
					アトリエフィッティング・コンサルテーション、またお受け取り時には、下記の通り感染予防対策など徹底しておりますが、ご来店は引き続き、自己判断に基づいてご決定くださいますようお願いいたします。<br>また発熱(37.5度以上)や倦怠感や咳・咽頭痛などの体調不良などの症状があるお客様、来店日から過去２週間以内に海外渡航されたお客様は、ご来店をお控えくださいますようお願いいたします。<br>キャンセルをご希望される場合は、ご予約の2日前までにお知らせください。
				</td>
			</tr>
			<tr>
			...
			<tr>
				<td style="margin: 0; padding: 15px 0px; font-size: 14px; font-weight: normal; color: #000000; font-family: 'Noto Sans JP', sans-serif; , serif; line-height: 24px; mso-line-height-rule: exactly;"
					align="left" valign="top"><strong>【Chiyono Anneアトリエの新型コロナウィルス感染拡大防止対策】</strong></td>
			</tr>
			<tr>
				<td style="height:15px;">&nbsp;</td>
			</tr>
			<tr>
				<td style="margin: 0; padding-bottom: 0; font-size: 14px; font-weight: normal; color: #000000; font-family: 'Noto Sans JP', sans-serif; , serif; line-height: 24px; mso-line-height-rule: exactly;"
					align="left" valign="top">• 緊急事態宣言期間中のアトリエご予約日は千代乃を含むアトリエスタッフ2名へのスタッフ人数削減
