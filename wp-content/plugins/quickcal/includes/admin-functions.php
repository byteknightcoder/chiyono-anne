...
function booked_parse_readme_changelog( $readme_url = false, $title = false ){
	...
	$readme = ( !$readme_url ? file_get_contents( QUICKCAL_PLUGIN_DIR . '/readme.txt') : file_get_contents( $readme_url ) );
...
function booked_render_time_select($select_name,$interval,$placeholder,$single = false){
	...
	$html .= '<option value="">'.$placeholder.'</option>';
		...
		do {
			$time_display = quickcal_convertTime($time);
			...
			echo '<div class="timeslot bookedClearFix" data-appt-id="'.$appointment['post_id'].'">';
				...
				echo '<span class="timeslot-count count-wide">';
					...
					echo '<span class="appt-block appt-no-padding" data-appt-id="'.$appointment['post_id'].'">';
						...
						do_action('booked_admin_calendar_buttons_before', $calendar_id, $appointment['post_id'], $status);

						if ( apply_filters('booked_admin_show_calendar_buttons', true) ) {
							echo '<a href="#" class="delete"'.($calendar_id ? ' data-calendar-id="'.$calendar_id.'"' : '').'><i class="fa-solid fa-xmark"></i></a>'.($status == 'pending' ? '<button data-appt-id="'.$appointment['post_id'].'" class="approve button button-primary">'.esc_html__('Approve','booked').'</button>' : '');
						}

						do_action('booked_admin_calendar_buttons_after', $calendar_id, $appointment['post_id'], $status);
