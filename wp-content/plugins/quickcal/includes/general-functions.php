...
function quickcal_get_name( $user_id, $part = 'full', $is_reversed = false ){
	switch ($part):
		case 'full':
			...
			if (!$username):
				...
				if ( !empty($user_info) ) {
					...
					$name_parts = explode(' ', $display_name); // Split the display name by spaces
					$username = implode(' ', array_reverse($name_parts)); // Reverse the parts
				} else {
					return false;
				}
			endif;
