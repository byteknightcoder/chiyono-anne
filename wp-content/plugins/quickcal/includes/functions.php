...
function booked_fe_calendar_date_content($date,$calendar_id = false){
	...
	echo '<div class="booked-appt-list">';
				...
				*/
				...
				if ($spots_available && $available || !$hide_unavailable_timeslots):
					...
					if ($hide_unavailable_timeslots && !$available):
						...
						$button_text = (!$spots_available || !$available ? esc_html__('Unavailable','booked') : esc_html( _x('予約する','Book a Single Appointment', 'booked') ) );
                                                if (
                                                    !empty($_POST['app_id'])
		...
		*/
		...
		echo '<h2><span>' . sprintf( _n( esc_html(_x( '%sの空き予約状況', 'Single Appointment', 'booked' )), esc_html(_x( '%sの空き予約状況', 'Multiple Appointments', 'booked' )), $total_available ), '</span><strong>'.$date_display.'</strong><span>') . '</span></h2>';
		echo $appt_list_html;
	...
	echo '<div class="booked-appt-list" data-list-date="'.$date.'" data-min-date="'.($prevent_before ? date_i18n('Y-m-d',strtotime($prevent_before)) : $earliest_date).'" data-max-date="'.($prevent_after ? date_i18n('Y-m-d',strtotime($prevent_after)) : false).'">';
		...
		*/
		...
		echo '<div class="booked-appt-list-header">';
			echo '<h2'.(!$showing_prev ? ' class="booked-no-prev"' : '').'><span>'.sprintf(_n('%sの空き予約状況','%sの空き予約状況',$total_available,'booked'),'</span><strong>'.$date_display.'</strong>').'</h2>';
