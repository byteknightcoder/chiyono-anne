...
if (!class_exists('quickcal_plugin')) {
	...
	{
		public $booked_screens = array('booked-pending', 'booked-appointments', 'booked-settings', 'booked-welcome');
		...
		public static function admin_reminders()
		{
			...
			// Start Modification by Amin Shoukat
			// Get reminder buffers in seconds
			$reminder_24_hours = 24 * 60 * 60; // 24 hours in seconds
			// Get current timestamp
			$current_timestamp = current_time('timestamp');
			
			// Calculate the time ranges
			$start_timestamp_24hr = $current_timestamp + $reminder_24_hours;
			
			// Arguments for 24-hour reminder query
			$args_24hr = array(
				...
				'meta_query'     => array(
					array(
						...
						'value'   => array( $current_timestamp, $start_timestamp_24hr ),
						'compare' => 'BETWEEN',
						'type'    => 'NUMERIC'
					)
				)
			...
			// Get post IDs for 24-hour reminders
			$appointment_ids_24hr = get_posts($args_24hr);
			...
			// Email content and email subject not empty
			if ( $email_content && $email_subject ) {
				// Send emails for 24-hour reminder appointments
				foreach ($appointment_ids_24hr as $appt_id) {
					// Send 24-hour reminder email
					$reminder_24hr_sent = get_post_meta($appt_id, '_appointment_admin_reminder_24hr', true);
					$reminder_1hr_sent = get_post_meta($appt_id, '_appointment_admin_reminder_1hr', true);
					$appointment_timestamp = get_post_meta($appt_id, '_appointment_timestamp', true);
					...
					if ( !$reminder_24hr_sent && $time_diff <= 24 && $time_diff >= 23 ) {
						update_post_meta($appt_id, '_appointment_admin_reminder_24hr', true);
						$is_email = true;

					}
					
					// If 24 hours reminder
					if ( !$reminder_1hr_sent && $time_diff >= 0 && $time_diff <= 1  ) {
						update_post_meta($appt_id, '_appointment_admin_reminder_1hr', true);
						$is_email = true;
					}

					if ( $is_email ) {
						$admin_email = quickcal_which_admin_to_send_email($calendar_id);
						$token_replacements = quickcal_get_appointment_tokens($appt_id, true);
			...
			// End Modification by Amin Shoukat
		...
		public static function user_reminders()
		{
			...
			if ( $email_content && $email_subject ) {
				...
				foreach ($appointment_ids_24hr as $appt_id) {
					// Send 24-hour reminder email
					$reminder_24hr_sent = get_post_meta($appt_id, '_appointment_user_reminder_24hr', true);
					$reminder_1hr_sent = get_post_meta($appt_id, '_appointment_user_reminder_1hr', true);
					$appointment_timestamp = get_post_meta($appt_id, '_appointment_timestamp', true);
					$seconds_diff = $appointment_timestamp - $current_timestamp;                            
					$time_diff = ( $seconds_diff / 3600);
					$is_email = false;
					...
					// If 24 hours reminder
					if ( !$reminder_24hr_sent && $time_diff <= 24 && $time_diff >= 23 ) {
						update_post_meta($appt_id, '_appointment_user_reminder_24hr', true);
						$is_email = true;
					...
					// If 24 hours reminder
					if ( !$reminder_1hr_sent && $time_diff >= 0 && $time_diff <= 1  ) {
						update_post_meta($appt_id, '_appointment_user_reminder_1hr', true);
						$is_email = true;
					}
		...
		public function booked_add_pending_appt_bubble()
		{
			...
			foreach ($submenu as $key => $value):
				if ($key == 'booked-appointments'):
					if ($pending) {
						$submenu[$key][1][0] .= "&nbsp;<span class='awaiting-mod count-$pending' title='$pending'><span class='pending-count' aria-hidden='true'>$pending</span><span class='comments-in-moderation-text screen-reader-text'>$pending Pending Bookings</span></span>";
					}
					return;
				endif;
			endforeach;
		...
		public function booked_calendars_edit_custom_fields($tag)
		{
			...
			<tr class="form-field">
				...
				<td>
					...
					<span
						class="description"><?php esc_html_e('This will use your setting from the QuickCal Settings panel by default.'); ?></span>
				</td>
			</tr>
			<?php
		}
...
function personal_information_form_shortcode()
{
	...
	$output = '<form id="personal-information-form" method="post">';
	...
	foreach ($custom_fields as $field) {
		...
		$asterisk = $is_required ? '<i class="required-asterisk fa-solid fa-asterisk"></i>' : '';

		$output .= '
        <div class="field">
            <label class="field-label" for="' . esc_attr($field['name']) . '">' . esc_html($field['value']) . $asterisk . '</label>
            <input id="' . esc_attr($field['name']) . '" ' . $required_attr . ' type="text" name="' . esc_attr($field['name']) . '" value="' . esc_attr($field_value) . '" class="large textfield" />
        </div>';
	}
	...
	$output .= '
        ...
        ' . wp_nonce_field('update_personal_information', 'personal_information_form_nonce', true, false) . '
    </form>';

	// Add JavaScript to handle form submission with page refresh
	$output .= "
    <script>
    jQuery(document).ready(function($) {
        $('#personal-information-form').on('submit', function(e) {
            ...
            formData += '&action=update_personal_information'; // Add action for WordPress AJAX handling
...
add_shortcode('personal_information_form', 'personal_information_form_shortcode');

// AJAX handler for form submission
function handle_personal_information_form_submission()
{
	check_ajax_referer('update_personal_information', 'personal_information_form_nonce');

	if (!is_user_logged_in()) {
		wp_send_json_error(['message' => 'ユーザーがログインしていません。']);
	}
	...
	wp_send_json_success(['message' => '個人情報が正常に更新されました。']);
}
add_action('wp_ajax_update_personal_information', 'handle_personal_information_form_submission');
