        ...
        if ($name_requirements == 'require_surname' && !$surname):
            ...
            if ($email && is_email($email) && $name || !$email && !$email_required && $name):
                ...
                if(isset($_SESSION['is_booked_try_fit_your_size'])&&$_SESSION['is_booked_try_fit_your_size']=='yes'){
                    $email_content = get_option('booked_appt_confirmation_email_content_try_fit_your_size', false);
                }elseif(isset($_SESSION['is_booked_special'])&&$_SESSION['is_booked_special']=='yes'&&$_POST['calendar_id']==438){////special-appointment
                    $email_content = get_option('booked_appt_confirmation_email_content_special', false);
                }elseif(booked_in_calendar($post_id, 'bspkmonth-offline')){
                    $email_content = get_option('booked_bspkmonth_offline_confirmation', false);
                }elseif(booked_in_calendar($post_id, 'bspkmonth-online')){
                    $email_content = get_option('booked_bspkmonth_online_confirmation', false);
                }else{
...
//            else:
            ...
            if (empty($errors)):
                ...
                update_user_meta($user_id, 'billing_first_name_kana', $arr_your_info['billing_first_name_kana']);
                update_user_meta($user_id, 'billing_phone', $arr_your_info['phone']);
                if (isset($arr_your_info['account_birth_data'])) {
                    $data_birth=array(
                        'year'=>'',
                        'month'=>'',
                        'day'=>''
                    );
                    if(!empty($arr_your_info['account_birth_data'])){
                        $arr= explode("-", $arr_your_info['account_birth_data']);
                        $month=(int)$arr[1];
                        $day=(int)$arr[2];
                        $data_birth['year']=$arr[0];
                        $data_birth['month']=(string)$month;
                        $data_birth['day']=(string)$day;
                    }
                    update_user_meta($user_id, 'account_birth', $data_birth);
                }
