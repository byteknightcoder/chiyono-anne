...
function booked_fe_calendar($year = false,$month = false,$calendar_id = false,$force_calendar = false){
	...
	$prevent_before = apply_filters('booked_prevent_appointments_before',get_option('booked_prevent_appointments_before',false));
	$prevent_after = apply_filters('booked_prevent_appointments_after',get_option('booked_prevent_appointments_after',false));
        if (isset($_SESSION['order_id_booked'])) {
    $order_id = $_SESSION['order_id_booked'];
} else {
    $order_id = null; // or handle it in another way, such as assigning a default value or triggering an error.
}
	...
	if ($calendar_id == 'undefined'): $calendar_id = 0; endif;
        $calendar_info=get_term_by('id', $calendar_id, 'booked_custom_calendars');
    ...
    $allow_not_same_datetime=array('fitting-calendar','try-fit-your-size');
    if(!in_array($calendar_info->slug, $allow_not_same_datetime)){
        if ($calendar_id):
            $args['tax_query'] = array(
                    array(
                            'taxonomy' => 'booked_custom_calendars',
    ...
    }else{        // to limit single seat on same date/time only between Consultation and Online Consultation calendar
		if ($calendar_id):
			$args['tax_query'] = array(
				array(
					'taxonomy' => 'booked_custom_calendars',
...
///////get slot of day
		...
		if ($calendar_id):
			$booked_defaults2 = get_option('booked_defaults_'.$calendar_id);
			if (!$booked_defaults2):
				$booked_defaults2 = get_option('booked_defaults');
			endif;
		else :
			$booked_defaults2 = get_option('booked_defaults');
		endif;

		$formatted_date = date_i18n('Ymd',strtotime($check_year.'-'.$check_month.'-'.$date));
		$booked_defaults2 = booked_apply_custom_timeslots_details_filter($booked_defaults2,$calendar_id);
$day_name2 = date('D',strtotime($check_year.'-'.$check_month.'-'.$date));
		if (isset($booked_defaults2[$formatted_date]) && !empty($booked_defaults2[$formatted_date])):
			$todays_defaults = (is_array($booked_defaults2[$formatted_date]) ? $booked_defaults2[$formatted_date] : json_decode($booked_defaults2[$formatted_date],true));
		elseif (isset($booked_defaults2[$formatted_date]) && empty($booked_defaults2[$formatted_date])):
			$todays_defaults = false;
		elseif (isset($booked_defaults2[$day_name2]) && !empty($booked_defaults2[$day_name2])):
			$todays_defaults = $booked_defaults2[$day_name2];
		else :
			$todays_defaults = false;
		endif;
                $is_booked_class='';
if ($todays_defaults):
                        $is_booked_class = 'booked prev-date';
        foreach ($todays_defaults as $timeslot => $count) {
        // Check if the key exists in the $appointments_array before accessing it
        $key = $year.$check_month.$date;
                            if (isset($appointments_array[$key]) && !isBookedByOtherCustomer($appointments_array[$key], $timeslot, $calendar_id)):
                                $is_booked_class = '';
                                break;
        endif;
    }
endif;           
                //block 1 week, that mean customer only book from paid date + 7
                $passcode_order = get_post_meta($order_id, '_ch_passcode', true);
                $passcode=isset($_REQUEST['passcode']) ? $_REQUEST['passcode'] : '';
                $valid_passcode=false;
                if($passcode_order==$passcode){
                    $valid_passcode=true;
                }
                $class_allow_booking='';
                if(!empty($passcode)&&!empty($order_id)&&$valid_passcode==true){//only for TBTY calendar
                    $date_allow_booking = date_i18n('Y-m-d 00:00:00', strtotime('+7 days', strtotime($_paid_date)));
                    $ch_date=$check_year.'-'.$check_month.'-'.$date;
                    if(strtotime($date_allow_booking)> strtotime($ch_date)){
                        $class_allow_booking=' prev-date';
                    }
                }
//                if(!current_user_can('administrator')){
//                    if(isset($_SESSION['is_booked_special'])){//for special calendar, this mean Event
//                        $ch_date=$check_year.'-'.$check_month.'-'.$date;
//                        if( strtotime($ch_date)<strtotime('2021-10-28')|| strtotime($ch_date)>strtotime('2021-11-01')){
//                            $class_allow_booking=' prev-date';
//                        }
//                    }
//                }
///////////end
						$html = '<td data-date="'.$check_year.'-'.$check_month.'-'.$date.'" class="'.implode(' ',$classes).$is_booked_class.$class_allow_booking.'">';
...
function isBookedByOtherCustomer($booked,$slot,$calendar_id=null){
    if(empty($booked)){
        return false;
    }
    $calendar_info=get_term_by('id', $calendar_id, 'booked_custom_calendars');
    ...
    //file_put_contents(dirname(__FILE__).'/checked.txt', $slot.'='.json_encode($booked)."\n");
//  1100-1120=
//  10:30-10:50
//  1030-1200====
    //{"1330-1500":1,"1530-1700":1,"1030-1200":1}
    //{"1330-1500":1,"1530-1700":1,"1030-1200":1}
    $arr_slot= explode("-", $slot);
    foreach ($booked as $key => $value) {
        $arr_slot_booked= explode("-", $value['timeslot']);
        $start_booked=$arr_slot_booked[0];
        $end_booked=$arr_slot_booked[1];
        $start_slot=$arr_slot[0];
        $end_slot=$arr_slot[1];
        if(($start_slot>=$start_booked&&$start_slot<=$end_booked)||($start_booked>=$start_slot&&$start_booked<=$end_slot)){
            return true;
        }
    }
    return false;
...
function booked_fe_calendar_date_content($date,$calendar_id = false){
	...
	echo '<div ch_changed class="booked-appt-list">';
...
//this commend to get all bookeds from all calendar
    $calendar_info=get_term_by('id', $calendar_id, 'booked_custom_calendars');
    $allow_not_same_datetime=array('fitting-calendar','try-fit-your-size');
    if(!in_array($calendar_info->slug, $allow_not_same_datetime)){
        if ($calendar_id):
				...
				if ($spots_available && $available || !$hide_unavailable_timeslots):
					...
					if ($hide_unavailable_timeslots && !$available):
						$html = '';
                                            ...
                                            if(!isBookedByOtherCustomer($appointments_array, $timeslot,$calendar_id)):
                                                $NON_AVAILABLE=true;
...
if($NON_AVAILABLE){
    ...
    echo '<h2 class="non_available"><span>この日にちは予約枠の空きはありません</span></h2>';
}
		echo $appt_list_html;
...
//function check_require($custom_fields,$numbers_only){
//    $require=false;
//    foreach ($custom_fields as $field){
//        if($field['name']=='required---'.$numbers_only && ($field['value']=='true'||$field['value']==true)){
//            $require=true;
//            break;
//        }
//    }
...
function booked_custom_fields($calendar_id = false){
	...
	if (!empty($custom_fields)):
		...
		foreach($custom_fields as $field):
            ...
            $is_required = false;//(isset($end_of_string[1]) ? true : false);
            if(check_require($custom_fields, $numbers_only)){
                $is_required=true;
            }
			if ($look_for_subs):
	...
	if ( isset( $_SERVER['HTTP_REFERER'] ) && isset( $_POST['booked_login_form'] ) ):
		...
		if ( !isset($_REQUEST['woocommerce-login-nonce']) && !empty($referrer) && !strstr($referrer,'wp-login') && !strstr($referrer,'wp-admin') ) {
                    if(isset($_SESSION['is_booked_special']) && $_SESSION['is_booked_special']=='yes'){
                        $secure='?secure='.$_SESSION['ch_book_secure'].'&loginfailed';
                    }elseif(isset($_SESSION['is_bespokemonth']) && $_SESSION['is_bespokemonth']=='yes'){
                        $secure='?secure='.$_SESSION['ch_book_secure_bes'].'&loginfailed';
                    }else if(isset($_SESSION['is_booked_try_fit_your_size']) && $_SESSION['is_booked_try_fit_your_size']=='yes'){
                        $secure='?passcode='.$_SESSION['ch_book_passcode'].'&order_id='.$_SESSION['ch_book_order_id'].'&loginfailed';
                    }else{
                        $secure='?loginfailed';
                    }
                    global $post;
                    $referrer=home_url('reservation-form').$secure;
...
function booked_send_user_approved_email($appt_id){
                ...
                $type=get_post_meta($appt_id, '_ch_booked_type', true);
                    if(isset($order_id)&&$order_id>0){
                        $email_content = get_option('booked_approval_email_content_try_fit');
                    }else{
                        if($type=='special'){
                            $email_content = get_option('booked_approval_email_content_special');
                        }elseif(booked_in_calendar($appt_id, 'bspkmonth-offline')){
                            $email_content = get_option('booked_bspkmonth_offline_approval', false);
                        }elseif(booked_in_calendar($appt_id, 'bspkmonth-online')){
                            $email_content = get_option('booked_bspkmonth_online_approval', false);
                        }else{
