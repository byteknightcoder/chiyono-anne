                ...
                while ($bookedAppointments->have_posts()):
                    ...
                    if (!$reminder_sent && apply_filters('booked_prepare_sending_reminder', true, $appt_id)):
                        ...
                        if ($email_content && $email_subject):
                            ...
                            $email_content = str_replace("%covid_notify%", $covid_notify, $email_content);
                            $email_content = str_replace("%notice_tbyb_zoom%", $notice_tbyb_zoom, $email_content);
                            $email_content = str_replace("%homeurl%", $homeurl, $email_content);
            ...
            if ($bookedAppointments->have_posts()):
                while ($bookedAppointments->have_posts()):
                    ...
                    if(isset($order_id)&&$order_id>0){
                        $email_content = get_option('booked_reminder_email_try_fit_your_size', false);
                    }else{
                        if($type=='special'){
                            $email_content = get_option('booked_reminder_email_special', false);
                        }elseif(booked_in_calendar($appt_id, 'bspkmonth-offline')){
                            $email_content = get_option('booked_bspkmonth_offline_reminder', false);
                        }elseif(booked_in_calendar($appt_id, 'bspkmonth-online')){
                            $email_content = get_option('booked_bspkmonth_online_reminder', false);
                        }else{
...
if (!session_id()) {
    ...
    if (headers_sent() === false) {
        session_start(); // Start the session if headers are not sent yet
    } else {
        ob_start(); // Start output buffering to prevent 'headers already sent' issues
        session_start(); // Start session
        ob_end_clean(); // Clean the output buffer and proceed
    }
}
    ...
    if (version_compare($current_version, BOOKED_VERSION) < 0 && !BOOKED_DEMO_MODE) {
        ...
        static function plugin_settings() {
            $plugin_options = array(
                ...
                'booked_reminder_email_try_fit_your_size',
                ...
                'booked_cancellation_email_content_try_fit_your_size',
                'booked_cancellation_email_subject',
                'booked_appt_confirmation_email_content',
                'booked_appt_confirmation_email_content_try_fit_your_size',
                ...
                'booked_cancellation_email_content_special',
                'booked_reminder_email_special',
                'booked_appt_confirmation_email_content_special',
                'booked_approval_email_content_try_fit',
                'booked_approval_email_content_special',
                'booked_bspkmonth_offline_reminder',//for bspkmonth-offline
                'booked_bspkmonth_offline_confirmation',
                'booked_bspkmonth_offline_approval',
                'booked_bspkmonth_offline_cancellation',
                'booked_bspkmonth_online_reminder',//for bspkmonth-online
                'booked_bspkmonth_online_confirmation',
                'booked_bspkmonth_online_approval',
                'booked_bspkmonth_online_cancellation',
            );
...
//save book type to booked
function ch_booked_new_appointment_created_save($post_id) {
    $_SESSION['booked_id']=$post_id;//save booked id, that just created to use shortcode in thanks page
    if (isset($_SESSION['is_booked_special']) && !empty($_SESSION['is_booked_special']) && $_SESSION['is_booked_special']=='yes') {
        $terms = get_the_terms( $post_id ,'booked_custom_calendars');
        if(!empty($terms)){
            foreach ( $terms as $term ) {
                if($term->term_id==438){
                    update_post_meta($post_id, '_ch_booked_type', 'special'); //save book type to booked
                    $_SESSION['is_booked_special']='no';
                    unset($_SESSION['is_booked_special']);
                }
            }
        }
    }
...
add_action('booked_new_appointment_created', 'ch_booked_new_appointment_created_save', 99, 1);
//check booked in calendar or not
function booked_in_calendar($post_id,$calendar_slug){
    $terms = get_the_terms( $post_id ,'booked_custom_calendars');
    $rest=false;
    if(!empty($terms)){
        foreach ( $terms as $term ) {
            if($term->slug==$calendar_slug){
                $rest=true;
                break;
            }
        }
...
add_shortcode('booked_calendar_name', 'ch_booked_calendar_name');

function ch_booked_calendar_name($atts)
{
    if (!$_SESSION['booked_id'])
        return '';
    $terms = get_the_terms( $_SESSION['booked_id'] ,'booked_custom_calendars');
    $calendar_name='';
    if(!empty($terms)){
        foreach ( $terms as $term ) {
            $calendar_name=$term->name;
            break;
        }
    }
...
function lastmsg_reservation($atts){
    if(!empty($atts['for-bspkmonth-online'])&&booked_in_calendar($_SESSION['booked_id'],'bspkmonth-online')){
        return $atts['for-bspkmonth-online'];
    }else if($atts['for-other']){
        return $atts['for-other'];
    }
}
add_shortcode('lastmsg_reservation','lastmsg_reservation');