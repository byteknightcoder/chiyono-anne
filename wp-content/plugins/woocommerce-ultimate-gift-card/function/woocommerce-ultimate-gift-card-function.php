	...
	class MWB_WGM_Card_Product_Function
	{
		...
		public function add_hooks_and_filters()
		{
			...
			if ($mwb_wgm_apply_coupon_disable == 'on') {
				add_action('wp_loaded', array($this, 'mwb_wgm_disable_coupons'), 20);
			}
			...
			//add_action('mwb_wgm_terms_and_condition', array($this,'mwb_wgm_terms_condition'));

			add_action('wp_enqueue_scripts', array($this, 'mwb_wgm_custom_css_js'));

		}

		public function mwb_wgm_disable_coupons()
		{
			add_filter('woocommerce_coupons_enabled', array($this, 'mwb_wgm_hidding_coupon_field_on_cart'), 10, 1);
		}


		public function mwb_wgm_custom_css_js()
		{
			wp_enqueue_style('mwb_wgm_custom_css', MWB_WGM_URL . 'assets/css/mwb-wgm-custom.css');
			//wp_register_script("mwb_wgm_refund_script", MWB_WGM_URL."/assets/js/mwb-refund-request.js", array('jquery'));
			//wp_localize_script('mwb_wgm_refund_script', 'mwb_wgm', $mwb_wgm );
			//wp_enqueue_script('mwb_wgm_refund_script' );
		}
		...
		public function mwb_wgm_hide_quantity_fields($return, $product)
		{
			...
			// if( $product_type == 'wgm_gift_card' && $mwb_wgm_disable_quantity_field == 'on') {
			// 	return true;
			// }
									...
									if ($itemgiftsend != "send") {
										...
										if ($mwb_wgm_sendtoday_disable == 'off') {
											...
											<div id="mwb_send_force_div_<?php echo $item_id; ?>">
												<input type="button" data-id="<?php echo $order_id; ?>" data-num="<?php echo $item_id; ?>"
													class="mwb_wgm_send_mail_force" class="button button-primary"
													value="<?php _e('Send Today', 'woocommerce-ultimate-gift-card'); ?>">
											</div>
											<?php
										}
									}
		...
		function mwb_wgm_woocommerce_add_to_cart_validation($validate, $product_id, $quantity)
		{
			...
			$mwb_wgm_remove_validation_to_name = get_option('mwb_wgm_remove_validation_to_name', false);
					...
					if ($product_type == 'wgm_gift_card') {
						...
						if ($mwb_wgm_preview_disable == 'off' && $is_customizable != 'yes') {
							...
							<span class="mwb_price">
								<a href="<?php echo home_url("?mwb_wgm_preview_email=mwb_wgm_preview_email&product_id=$product_id") ?>&TB_iframe=true&width=630&height=500"
									class="thickbox"><?php _e('Preview', 'woocommerce-ultimate-gift-card'); ?></a>
							</span>
							<?php
						}
					}
		...
		function mwb_wgm_preview_mail()
		{
			...
			if ($browse_enable == "on") {
				...
				if (
					isset($_FILES["file"]) && (($_FILES["file"]["type"] == "image/gif")
				...
				if ($product_type == 'wgm_gift_card' && $mwb_wgm_hide_giftcard_notice == 'off') {
					...
					if ($giftcard_expiry > 0) {
						...
						<div class="mwb_wgm_expiry_notice clear">
							...
							<p><?php echo sprintf(__('This gift card will expire  %s days after purchase.', 'woocommerce-ultimate-gift-card'), $days); ?>
							</p>
						</div>
						<?php
			...
			if ($order_status == 'completed' || $order_status == 'processing') {
					...
					if (isset($delivery_method) && !empty($delivery_method) && $delivery_method == 'Downloadable') {
						...
						if (isset($giftcoupon) && !empty($giftcoupon)) {
							foreach ($giftcoupon as $key => $value) {
								...
								?>
								<a style="margin: 5px;" href="<?php echo $uploadDirPath; ?>"
									target="_blank"><?php _e('Download PDF', 'woocommerce-ultimate-gift-card'); ?></a><br />
								<?php
							}
						}
					}
				...
				if ($giftcard && $resend_view) {
					...
					if ($mwb_wgm_resend_disable == 'off') {
						?>
						<header>
							<h4><?php _e('Resend Giftcard Email', 'woocommerce-ultimate-gift-card') ?></h4>
						</header>
						<div id="mwb_wgm_loader" style="display: none;">
							<img src="<?php echo MWB_WGM_URL ?>/assets/images/loading.gif">
						</div>
						<p><?php _e('If the recipient has not received your giftcard email then you can resend the email.', 'woocommerce-ultimate-gift-card'); ?>
						</p>
						<p id="mwb_wgm_resend_mail_notification"></p>
						<input type="button" data-id="<?php echo $order_id; ?>" id="mwb_wgm_resend_mail_button" class="button button-primary"
							value="<?php _e('Resend Mail', 'woocommerce-ultimate-gift-card'); ?>">
						<?php
					}
				}
			}
			...
			if (is_product()) {
				...
				if (isset($product_types[0])) {
					...
					if ($product_type == 'wgm_gift_card') {
						...
						$mwb_wgm_remove_validation_to_name = get_option('mwb_wgm_remove_validation_to_name', 'off');
									...
									<div class="mwb_wgm_added_wrapper">
										...
										if (isset($product_pricing['type'])) {
											...
											if ($product_pricing_type == 'mwb_wgm_range_price') {
												...
												if ($discount_applicable) { ?>
													<div class="mwb_wgm_price_content">
														...
														<b style="color:green;"><?php _e('Discounted Gift Card Price: ', 'woocommerce-ultimate-gift-card');
														echo wc_price($new_price_range); ?></b><br />
														<b style="color:green;"><?php _e('Giftcard Value: ', 'woocommerce-ultimate-gift-card');
														echo wc_price($text_box_price); ?></b>
												...
												<p class="mwb_wgm_section">
													...
													<input type="number" class="input-text mwb_wgm_price" id="mwb_wgm_price" name="mwb_wgm_price"
														value="<?php echo ($default_price >= $from_price && $default_price <= $to_price) ? $default_price : $from_price; ?>"
														max="<?php echo $to_price; ?>" min="<?php echo $from_price; ?>">
											...
											if ($product_pricing_type == 'mwb_wgm_default_price') {
												$default_price = $product_pricing['default_price'];
												?>
												<input type="hidden" class="mwb_wgm_price" id="mwb_wgm_price" name="mwb_wgm_price"
													value="<?php echo $default_price ?>">
												<?php
												if (isset($discount_enable) && $discount_enable == 'on') {
													if (isset($mwb_wgm_discount[0]) && $mwb_wgm_discount[0] == 'yes') { ?>

														<span style="color:green;"><?php _e('Coupon Amount will be: ', 'woocommerce-ultimate-gift-card');
														echo $default_price; ?></span>
											...
											if ($product_pricing_type == 'mwb_wgm_user_price') {
												...
												if ($discount_applicable) {
													...
													<div class="mwb_wgm_price_content">
														...
														<b style="color:green;"><?php _e('Discounted Gift Card Price: ', 'woocommerce-ultimate-gift-card');
														echo wc_price($new_price_user); ?></b><br />
														<b style="color:green;"><?php _e('Giftcard Value: ', 'woocommerce-ultimate-gift-card');
														echo wc_price($default_price); ?></b>
										...
										if ($giftcard_selected_date == "on") {
											...
											<p class="mwb_wgm_section demo_theme_date_style">
												...
												<span
													class="mwb_wgm_info"><?php _e('(Recipient will receive the gift card on selected date)', 'woocommerce-ultimate-gift-card'); ?></span>
										...
										<p class="mwb_wgm_section">
											...
											<input type="text" name="mwb_wgm_from_name" id="mwb_wgm_from_name" class="mwb_wgm_from_name"
												placeholder="<?php _e('Enter the sender name', 'woocommerce-ultimate-gift-card'); ?>" required="required">
										</p>
										<p class="mwb_wgm_section">
										...
										<p>
											...
											<div class="mwb_wgm_delivery_method">
												<input type="radio" name="mwb_wgm_send_giftcard" value="Mail to recipient" class="mwb_wgm_send_giftcard"
													checked="checked" id="to_mail"><span
													class="mwb_wgm_method"><?php echo $mwb_wgm_mail_to_recipient_text; ?></span>
												<div class="mwb_wgm_delivery_via_email">
													<input type="text" name="mwb_wgm_to_email" id="mwb_wgm_to_email" class="mwb_wgm_to_email"
														placeholder="<?php _e('Enter the Recipient Email (Required)', 'woocommerce-ultimate-gift-card'); ?>">
													<input type="text" name="mwb_wgm_to_name_optional" id="mwb_wgm_to_name_optional" class="mwb_wgm_to_email"
														placeholder="<?php _e('Enter the Recipient Name', 'woocommerce-ultimate-gift-card'); ?>"><span
														class="mwb_wgm_msg_info"><?php echo $mwb_wgm_mail_to_recipient_desc; ?></span>
												</div>
										...
										if (isset($mwb_wgm_method_enable) && $mwb_wgm_method_enable == 'download') {
											?>
											<div class="mwb_wgm_delivery_method">
												<input type="radio" name="mwb_wgm_send_giftcard" value="Downloadable" class="mwb_wgm_send_giftcard"
													checked="checked" id="mwb_wgm_send_giftcard_download"><span
													class="mwb_wgm_method"><?php echo $mwb_wgm_downloadable_text; ?></span>
												<div class="mwb_wgm_delivery_via_buyer">
													<input type="text" name="mwb_wgm_to_email_name" id="mwb_wgm_to_download" class="mwb_wgm_to_email"
														placeholder="<?php _e('Enter the Recipient Name', 'woocommerce-ultimate-gift-card'); ?>"><span
														class="mwb_wgm_msg_info"><?php echo $mwb_wgm_downloadable_desc; ?></span>
												</div>
											</div>
											<?php
										}
										...
										if (isset($mwb_wgm_method_enable) && $mwb_wgm_method_enable == 'shipping') {
											...
											<div class="mwb_wgm_delivery_method">
												...
												<input type="radio" name="mwb_wgm_send_giftcard" value="Shipping" class="mwb_wgm_send_giftcard"
													checked="checked" id="mwb_wgm_send_giftcard_ship"><span
													class="mwb_wgm_method"><?php echo $mwb_wgm_shipping_text; ?></span>
												<div class="mwb_wgm_delivery_via_admin">
													<input type="text" name="mwb_wgm_to_email_ship" id="mwb_wgm_to_ship" class="mwb_wgm_to_email"
														placeholder="<?php _e('Enter the Recipient Name', 'woocommerce-ultimate-gift-card'); ?>"><span
														class="mwb_wgm_msg_info"><?php echo $mwb_wgm_shipping_desc; ?></span>
												</div>
											</div>
											<?php
										}
											...
											if (isset($mwb_wgm_is_overwrite) && $mwb_wgm_is_overwrite == 'yes') {
												if (isset($mwb_wgm_email_to_recipient) && $mwb_wgm_email_to_recipient == 'yes') {
													...
													<div class="mwb_wgm_delivery_method">
														...
														<input type="radio" name="mwb_wgm_send_giftcard" value="Mail to recipient" class="mwb_wgm_send_giftcard"
															id="to_mail" checked="checked"><span
															class="mwb_wgm_method"><?php echo $mwb_wgm_mail_to_recipient_text; ?></span>
														<div class="mwb_wgm_delivery_via_email">
															<input type="text" name="mwb_wgm_to_email" id="mwb_wgm_to_email" class="mwb_wgm_to_email"
																placeholder="<?php _e('Enter the Recipient Email', 'woocommerce-ultimate-gift-card'); ?>">
															<input type="text" name="mwb_wgm_to_name_optional" id="mwb_wgm_to_name_optional" class="mwb_wgm_to_email"
																placeholder="<?php _e('Enter the Recipient Name', 'woocommerce-ultimate-gift-card'); ?>">
												...
												if (isset($mwb_wgm_download) && $mwb_wgm_download == 'yes') {
													...
													<div class="mwb_wgm_delivery_method">
														...
														<input type="radio" name="mwb_wgm_send_giftcard" value="Downloadable" class="mwb_wgm_send_giftcard"
															id="mwb_wgm_send_giftcard_download"><span
															class="mwb_wgm_method"><?php echo $mwb_wgm_downloadable_text; ?></span>
														<div class="mwb_wgm_delivery_via_buyer">
															<input type="text" name="mwb_wgm_to_email_name" id="mwb_wgm_to_download"
																class="mwb_wgm_to_email mwb_wgm_disable"
																placeholder="<?php _e('Enter the Recipient Name', 'woocommerce-ultimate-gift-card'); ?>" readonly><span
																class="mwb_wgm_msg_info"><?php echo $mwb_wgm_downloadable_desc; ?></span>
														</div>
													</div>
													<?php
												...
												if (isset($mwb_wgm_shipping) && $mwb_wgm_shipping == 'yes') {
													...
													<div class="mwb_wgm_delivery_method">
														<input type="radio" name="mwb_wgm_send_giftcard" value="Shipping" class="mwb_wgm_send_giftcard"
															id="mwb_wgm_send_giftcard_ship">
														<span class="mwb_wgm_method"><?php echo $mwb_wgm_shipping_text; ?></span>
														<div class="mwb_wgm_delivery_via_admin">
															<input type="text" name="mwb_wgm_to_email_ship" id="mwb_wgm_to_ship"
																class="mwb_wgm_to_email mwb_wgm_disable"
																placeholder="<?php _e('Enter the Recipient Name', 'woocommerce-ultimate-gift-card'); ?>" readonly><span
																class="mwb_wgm_msg_info"><?php echo $mwb_wgm_shipping_desc; ?></span>
														</div>
												...
												if (isset($mwb_wgm_customer_selection['Email_to_recipient']) && $mwb_wgm_customer_selection['Email_to_recipient'] == '1') {
													...
													<div class="mwb_wgm_delivery_method">
														...
														<input type="radio" name="mwb_wgm_send_giftcard" value="Mail to recipient" class="mwb_wgm_send_giftcard"
															id="to_mail" checked="checked">
														<span class="mwb_wgm_method"><?php echo $mwb_wgm_mail_to_recipient_text; ?>
															<div class="mwb_wgm_delivery_via_email">
																<input type="text" name="mwb_wgm_to_email" id="mwb_wgm_to_email" class="mwb_wgm_to_email"
																	placeholder="<?php _e('Enter the Recipient Email', 'woocommerce-ultimate-gift-card'); ?>">
																<input type="text" name="mwb_wgm_to_name_optional" id="mwb_wgm_to_name_optional"
																	class="mwb_wgm_to_email"
																	placeholder="<?php _e('Enter the Recipient Name ', 'woocommerce-ultimate-gift-card'); ?>">
																<span class="mwb_wgm_msg_info"><?php echo $mwb_wgm_mail_to_recipient_desc; ?></span>
															</div>
													</div>
												...
												if (isset($mwb_wgm_customer_selection['Downloadable']) && $mwb_wgm_customer_selection['Downloadable'] == '1') {
													...
													<div class="mwb_wgm_delivery_method">
														...
														<input type="radio" name="mwb_wgm_send_giftcard" value="Downloadable" class="mwb_wgm_send_giftcard"
															id="mwb_wgm_send_giftcard_download">
												...
												if (isset($mwb_wgm_customer_selection['Shipping']) && $mwb_wgm_customer_selection['Shipping'] == '1') {
													...
													<div class="mwb_wgm_delivery_method">
														<input type="radio" name="mwb_wgm_send_giftcard" value="Shipping" class="mwb_wgm_send_giftcard"
															id="mwb_wgm_send_giftcard_ship"><span class="mwb_wgm_method"><?php echo $mwb_wgm_shipping_text; ?></span>
														<div class="mwb_wgm_delivery_via_admin">
															<input type="text" name="mwb_wgm_to_email_ship" id="mwb_wgm_to_ship"
																class="mwb_wgm_to_email mwb_wgm_disable"
																placeholder="<?php _e('Enter the Recipient Name', 'woocommerce-ultimate-gift-card'); ?>" readonly><span
										...
										if ($gift_browse_enable == "on") {
											...
											<div class="mwb_demo_browse">
												...
												<p class="mwb_wgm_section">
													...
													<input type="file" name="mwb_wgm_browse_img" id="mwb_wgm_browse_img" class="mwb_wgm_browse_img"><span
														class="mwb_wgm_info"><?php _e('(Uploaded Image will replace the product image in template)', 'woocommerce-ultimate-gift-card'); ?></span>
										...
										if ($mwb_wgm_preview_disable == 'off') {
											?>
											<br /><span class="mwg_wgm_preview_email"><a id="mwg_wgm_preview_email"
													href="javascript:void(0);"><?php _e('Preview', 'woocommerce-ultimate-gift-card'); ?></a> </span><?php } ?>
										</p>
									</div>
									<?php
						...
						if (isset($_POST['mwb_wgm_to_email']) && !empty($_POST['mwb_wgm_to_email'])) {
							...
							if (isset($product_pricing) && !empty($product_pricing)) {
								...
								if ($browse_img == "on" && !empty($_FILES["mwb_wgm_browse_img"]["type"])) {
									...
									if (
										($_FILES["mwb_wgm_browse_img"]["type"] == "image/gif")
		...
		function mwb_wgm_woocommerce_get_item_data($item_meta, $existing_item_meta)
		{
			...
			//$is_customizable = get_post_meta($existing_item_meta['product_id'],'woocommerce_customizable_gift_card',true);
			// print_r($existing_item_meta);
			$mwb_wgm_enable = mwb_wgm_giftcard_enable();
			if ($mwb_wgm_enable) {
		...
		function mwb_wgm_create_gift_coupon($gift_couponnumber, $couponamont, $order_id, $product_id, $to)
		{
			...
			if ($mwb_wgm_enable) {
				...
				if ($alreadycreated != 'send') {
					...
					if (isset($mwb_wgm_exclude_per_category) && is_array($mwb_wgm_exclude_per_category[0]) && !empty($mwb_wgm_exclude_per_category[0])) {
					...
					if (isset($mwb_wgm_include_per_category) && is_array($mwb_wgm_include_per_category[0]) && !empty($mwb_wgm_include_per_category[0])) {
		...
		function mwb_wgm_giftttemplate($args)
		{
			...
			if ($mwb_wgm_enable) {
				...
				$templatehtml = apply_filters("mwb_wgm_email_template_html", $templatehtml, $product);
				...
				if (isset($values['product_meta'])) {
					foreach ($values['product_meta']['meta_data'] as $key => $val) {
						...
						if ($val) {
							...
							//do_action('mwb_wgm_add_meta_data',$item, $key, $order_val);	
						}
					}
				}
			}
		...
		public function mwb_wgm_attached_pdf($message, $site_name, $time, $order_id = '', $coupon_code = '')
		{
			...
			if ($mwb_wgm_new_way_of_pdf == 'yes' && $mwb_wgm_wkhtmltopdf) {
				...
				$handle = fopen(MWB_UPLOAD_DIR . '/giftcard_pdf/giftcard' . $time . $site_name . '.html', 'w');
				fwrite($handle, $giftcard_pdf_content);
				fclose($handle);
				$url = MWB_UPLOAD_URL . '/giftcard_pdf/giftcard' . $time . $site_name . '.html';
				...
				if ($mwb_wgm_pdf_template_size == 'A3') {
					...
					$result = exec(MWB_WGM_DIRPATH . 'wkhtmltox/bin/wkhtmltopdf --page-size A3 --encoding utf-8 ' . $url . ' ' . $uploadDirPath . '/giftcard' . $order_id . '-' . $item_id . '.pdf', $output, $return);
					...
					$result = exec(MWB_WGM_DIRPATH . 'wkhtmltox/bin/wkhtmltopdf --page-size A4 --encoding utf-8 ' . $url . ' ' . $uploadDirPath . '/giftcard' . $order_id . $coupon_code . '.pdf', $output, $return);
