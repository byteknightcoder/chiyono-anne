...
namespace Iconic_PC_NS\StellarWP\Uplink\Components\Admin;

use InvalidArgumentException;
use Iconic_PC_NS\StellarWP\Uplink\Auth\Admin\Disconnect_Controller;
use Iconic_PC_NS\StellarWP\Uplink\Auth\Auth_Url_Builder;
use Iconic_PC_NS\StellarWP\Uplink\Auth\Authorizer;
use Iconic_PC_NS\StellarWP\Uplink\Auth\Token\Contracts\Token_Manager;
use Iconic_PC_NS\StellarWP\Uplink\Components\Controller;
use Iconic_PC_NS\StellarWP\Uplink\Config;
use Iconic_PC_NS\StellarWP\Uplink\View\Contracts\View;
...
final class Authorize_Button_Controller extends Controller {
	...
	public function render( array $args = [] ): void {
		...
		if ( empty ( $slug ) ) {
			throw new InvalidArgumentException( __( 'The Product slug cannot be empty', 'jckpc' ) );
		}
		...
		if ( ! $this->authorizer->can_auth() ) {
			$target    = '_self';
			$link_text = __( 'Contact your network administrator to connect', 'jckpc' );
			$url       = get_admin_url( get_current_blog_id(), 'network/' );
		} elseif ( $this->token_manager->get() ) {
			$authenticated = true;
			$target        = '_self';
			$link_text     = __( 'Disconnect', 'jckpc' );
