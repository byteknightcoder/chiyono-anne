...
namespace Iconic_PC_NS\StellarWP\Uplink\Auth\Admin;

use Iconic_PC_NS\StellarWP\Uplink\Auth\Nonce;
use Iconic_PC_NS\StellarWP\Uplink\Auth\Token\Connector;
use Iconic_PC_NS\StellarWP\Uplink\Auth\Token\Exceptions\InvalidTokenException;
use Iconic_PC_NS\StellarWP\Uplink\Notice\Notice_Handler;
use Iconic_PC_NS\StellarWP\Uplink\Notice\Notice;
use Iconic_PC_NS\StellarWP\Uplink\Resources\Collection;
	...
	public function maybe_store_token_data(): void {
		...
		if ( ! Nonce::verify( $args[ self::NONCE ] ?? '' ) ) {
			$this->notice->add( new Notice( Notice::ERROR,
				__( 'Unable to save token data: nonce verification failed.', 'jckpc' ),
				true
			) );
		...
		if ( $license && $slug ) {
			if ( ! $this->collection->offsetExists( $slug ) ) {
				$this->notice->add( new Notice( Notice::ERROR,
					__( 'Plugin or Service slug not found.', 'jckpc' ),
					true
				) );
			...
			if ( ! $plugin->set_license_key( $license, 'network' ) ) {
				$this->notice->add( new Notice( Notice::ERROR,
					__( 'Error storing license key.', 'jckpc' ),
				true
				) );
		...
		$this->notice->add(
			new Notice( Notice::SUCCESS,
				__( 'Connected successfully.', 'jckpc' ),
				true
			)
		);
	}
