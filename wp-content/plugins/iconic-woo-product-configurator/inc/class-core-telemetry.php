...
if ( class_exists( 'Iconic_PC_Core_Telemetry' ) ) {
	return;
}

use Iconic_PC_NS\StellarWP\Telemetry\Core as Telemetry;
use Iconic_PC_NS\StellarWP\Telemetry\Config;
use Iconic_PC_NS\StellarWP\Telemetry\Opt_In\Status;

/**
 * Iconic_PC_Core_Telemetry.
 *
 * @class    Iconic_PC_Core_Telemetry
 * @version  1.0.0
 */
class Iconic_PC_Core_Telemetry {
	/**
	 * Single instance of the Iconic_PC_Core_Telemetry object.
	 *
	 * @var Iconic_PC_Core_Telemetry|null
	 */
	...
	/**
	 * Creates/returns the single instance Iconic_PC_Core_Telemetry object.
	 *
	 * @param array $args Arguments.
	 *
	 * @return Iconic_PC_Core_Telemetry
	 */
	public static function run( $args = array() ) {
		if ( null === self::$single_instance ) {
			$default_args = array(
				'settings_class'           => 'Iconic_PC_Core_Settings',
	...
	public static function optin_args( $args, $stellar_slug ) {
		...
		// Translators: The plugin name.
			__( 'Opt in to help us enhance %s', 'jckpc' ),
		...
		$args['permissions_url'] = 'https://iconicwp.com/telemetry-tracking/?utm_source=Iconic&utm_medium=Plugin&utm_campaign=opt-in&utm_content=jckpc';
		$args['tos_url']         = 'https://iconicwp.com/terms/?utm_source=Iconic&utm_medium=Plugin&utm_campaign=opt-in&utm_content=jckpc';
	...
	public static function exit_interview_args( $args, $stellar_slug ) {
		...
		$args['uninstall_reasons'][] = [
			...
			'uninstall_reason'    => __( "I'm just deactivating temporarily.", 'jckpc' ),
		];
	...
	public static function optin_setting( $settings = array() ) {
		...
		$settings['sections'][ self::$args['opt_out_settings_section'] ]['fields'][] = array(
			'id'       => 'optin',
			'title'    => __( 'Opt In', 'jckpc' ),
			// Translators: The telemetry terms URL.
			'subtitle' => sprintf( __( 'Help us improve this plugin by sharing <a href="%s" target="_blank">non-sensitive</a> data with us.', 'jckpc' ), 'https://iconicwp.com/telemetry-tracking/?utm_source=Iconic&utm_medium=Plugin&utm_campaign=opt-in-setting&utm_content=jckpc' ),
			'type'     => 'toggle',
			'default'  => $status,
		);
	...
	public static function save_optin_setting() {
		...
		if ( ! wp_verify_nonce( $nonce, self::$args['option_group'] . '-options' ) ) {
			wp_die( esc_html__( 'Sorry, you can\'t do that.', 'jckpc' ) );
		}
