...
class Iconic_PC_Inventory {
	 ...
	 * @param int $post_id
	 */
	public static function process_product_meta( $post_id, $post_data = false ) {
		$inventory_data = $post_data ? $post_data : ( isset( $_POST['jckpc_inventory'] ) ? $_POST['jckpc_inventory'] : false );
		...
		foreach ( $inventory_data as $att_val_id => $inventory ) {
	...
	public static function get_inventory( $args = array() ) {
		...
		/**
		 * Filter: modify args passed to get_inventory calls.
		 *
		 * @filter: iconic_pc_get_inventory_args
		 * @since 1.7.1
		 * @param array $args Arguments: product_id, att_val_id.
		 */
		$args       = apply_filters( 'iconic_pc_get_inventory_args', $args );
		$table_name = self::get_table_name();
	...
	public static function output_inventory_json() {
		...
		/**
		 * Filter: filter the Post/Product objects to iterate through
		 * when outputting inventory JSON data.
		 *
		 * @filter iconic_pc_inventory_products
		 * @since 1.7.0
		 * @param array $pc_products Array of WC_Product objects.
		*/
		$product_configurator_objects = apply_filters(
			'iconic_pc_inventory_products',
			array_filter(
				array( $product ),
				function( $value ) {
					return ( is_object( $value ) && is_a( $value, 'WC_Product' ) );
				}
			)
		);
		...
		if ( ! empty( $product_configurator_objects ) ) {
			foreach ( $product_configurator_objects as $product_configurator_object ) {
				$object_id = $product_configurator_object->get_id();

				if ( ! $object_id ) {
					continue;
				}

				$configurator_enabled = Iconic_PC_Product::is_configurator_enabled( $object_id );

				if ( ! $configurator_enabled ) {
					continue;
				}

				$attributes = Iconic_PC_Product::get_attributes( $object_id );
				...
				foreach ( $attributes as $att_slug => $att_data ) {
					...
					foreach ( $att_data['values'] as $value ) {
						...
						$json_array[ $object_id ][ $rel_select ][ $att_val_slug ] = $inventory;
					}
				}
			}
		}
		...
		echo '/* <![CDATA[ */ var jckpc_inventory = ' . wp_json_encode( $json_array ) . ' /* ]]> */';
		echo '</script>';
	}
