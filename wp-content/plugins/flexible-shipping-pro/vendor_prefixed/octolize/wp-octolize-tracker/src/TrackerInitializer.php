...
class TrackerInitializer implements \FSProVendor\WPDesk\PluginBuilder\Plugin\HookableCollection
{
     ...
     */
    public function __construct(string $plugin_file, string $plugin_slug, string $plugin_name, string $shop_url, \FSProVendor\Octolize\Tracker\OptInNotice\ShouldDisplay $should_display, \FSProVendor\WPDesk\Tracker\Deactivation\ReasonsFactory $reasons_factory = null)
    {
        ...
        $this->reasons_factory = $reasons_factory ?? new \FSProVendor\Octolize\Tracker\DeactivationTracker\OctolizeReasonsFactory();
    }
    /**
     * Hooks.
     *
    ...
    public function hooks()
    {
        ...
        $this->add_hookable(\FSProVendor\WPDesk\Tracker\Deactivation\TrackerFactory::createCustomTracker(new \FSProVendor\WPDesk\Tracker\Deactivation\PluginData($this->plugin_slug, $this->plugin_file, $this->plugin_name), null, null, null, $this->reasons_factory));
     ...
     */
    public static function create_from_plugin_info(\FSProVendor\WPDesk_Plugin_Info $plugin_info, $should_display, \FSProVendor\WPDesk\Tracker\Deactivation\ReasonsFactory $reasons_factory = null)
    {
        $shops = $plugin_info->get_plugin_shops();
        $shop_url = $shops[\get_locale()] ?? $shops['default'] ?? 'https://octolize.com';
        return new self($plugin_info->get_plugin_file_name(), $plugin_info->get_plugin_slug(), $plugin_info->get_plugin_name(), $shop_url, $should_display ?? new \FSProVendor\Octolize\Tracker\OptInNotice\ShouldDisplayAlways(), $reasons_factory);
    }
    /**
     ...
     * @return TrackerInitializer
     */
    public static function create_from_plugin_info_for_shipping_method(\FSProVendor\WPDesk_Plugin_Info $plugin_info, string $shipping_method_id, \FSProVendor\WPDesk\Tracker\Deactivation\ReasonsFactory $reasons_factory = null)
    {
        $shops = $plugin_info->get_plugin_shops();
        $shop_url = $shops[\get_locale()] ?? $shops['default'] ?? 'https://octolize.com';
