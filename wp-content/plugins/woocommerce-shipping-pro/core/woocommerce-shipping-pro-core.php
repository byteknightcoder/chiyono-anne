...
class wf_woocommerce_shipping_pro_method extends WC_Shipping_Method {
	...
	function calculate_shipping( $package = array() ) {
		...
		$this->wf_add_rate(apply_filters( 'wf_woocommerce_shipping_pro_shipping_costs', $costs, $this, $package),$package);
	...
	function wf_add_rate($costs,$package) {
		if ( sizeof($costs) > 0) {
			...
			foreach ($costs as $method_group => $method_cost) {
				if(true || $this->wf_check_all_item_exists($method_cost['cost'],$grouped_package)){
